...
void RaySpotLight::drawOpenGL(int index){
	...
	GLfloat dir[3] = { direction[0], direction[1], direction[2] };
	glLightfv(GL_LIGHT0 + index, GL_SPOT_DIRECTION, dir);
	GLfloat cutoff[1] = { cutOffAngle * 180 / PI};
	glLightfv(GL_LIGHT0 + index, GL_SPOT_CUTOFF, cutoff);
	GLfloat exp[1] = { dropOffRate };
	glLightfv(GL_LIGHT0 + index, GL_SPOT_EXPONENT, exp);
}